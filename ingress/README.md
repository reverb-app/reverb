...
Additionally, the ingress server serves as an API for the MongoDB database which hosts your logs. The following are the provided endpoints and their uses. For more information about the query params, see the [query params details table](#query-param-details). Note that this API follows the [HATEOAS](https://en.wikipedia.org/wiki/HATEOAS) convention and provides links for both collections and individual entries. The **Response Shape** column in the table below describes the properties on the JSON response you can expect to receive from each endpoint.

| Endpoint                  | Logs accessed                                                                                                                      | Allowed query params                                                                                | Response Shape                                                                                                                                                                              |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/logs`                   | All logs.                                                                                                                          | <ul><li>**startTime** and **endTime**</li><li>**limit**</li></ul>                                   | [ ... ]                                                                                                                                                                                     |
| `/logs/events`            | All events fired, either by a request being sent to the `/events` or `/webhooks` endpoints or those emitted by a **Reverb job**.   | <ul><li>**startTime** and **endTime**</li><li>**limit**</li><li>**page**</li></ul>                  | {<br/>&nbsp; &nbsp; logs: [<br/> &nbsp; &nbsp; &nbsp; &nbsp; event: { ... }, <br/>&nbsp; &nbsp; &nbsp; &nbsp; links: { ... }<br/>&nbsp; &nbsp; ],<br/>&nbsp; &nbsp; links: { ... }<br/>}    |
| `/logs/functions`         | All functions invoked and their statuses (running, completed, or error).                                                           | <ul><li>**startTime** and **endTime**</li><li>**limit**</li><li>**page**</li></ul>                  | {<br/>&nbsp; &nbsp; logs: [<br/> &nbsp; &nbsp; &nbsp; &nbsp; function: { ... }, <br/>&nbsp; &nbsp; &nbsp; &nbsp; links: { ... }<br/>&nbsp; &nbsp; ],<br/>&nbsp; &nbsp; links: { ... }<br/>} |
